...
use bincode::{deserialize_from, serialize_into, BincodeRead};
...
/// snapshot. bincode 1.2's standard `IoReader` reserves the declared length
/// before discovering EOF, so its aggregate byte limit alone does not prevent
/// an attacker-controlled length prefix from requesting tens of GiB.
...
// the full state from snapshot data files by imposing fixed aggregate and
// per-variable-field limits with ample headroom for valid snapshots.
