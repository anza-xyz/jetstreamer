...
// Keep snapshot extraction below a fixed power-of-two member ceiling. The
// qualified slot-64,799,804 boundary archive loads within this bound while
// paths, member sizes, aggregate size, sparse maps, and file types remain
...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.5.6-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // Exact v1.5.6 `bank_from_archive` schema. The upstream reconstruction
        ...
        .map_err(|error| format!("failed to decode v1.5.6 snapshot bank: {}", error))?;
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_upstream_v1_5_6_snapshot_schema() {
        assert_eq!(
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_5_6_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_64799804")
            .expect("set JETSTREAMER_SNAPSHOT_64799804 to run this integration test");
        let ledger =
        ...
        assert_eq!(loaded.bank.slot(), 64_799_804);
