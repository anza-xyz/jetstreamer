...
// The audited boundary snapshot contains 380,435 members at slot 63,935,659,
// almost entirely AppendVecs. The next power of two admits it with about 38%
...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.5.19-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // Exact v1.5.19 `bank_from_archive` schema. The upstream reconstruction
        ...
        .map_err(|error| format!("failed to decode v1.5.19 snapshot bank: {}", error))?;
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_upstream_v1_5_19_snapshot_schema() {
        assert_eq!(
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_5_19_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_63935659")
            .expect("set JETSTREAMER_SNAPSHOT_63935659 to run this integration test");
        let ledger =
        ...
        assert_eq!(loaded.bank.slot(), 63_935_659);
