...
// The first observed archive in the v1.2 era is at slot 26,351,963. Admit
// snapshots and entries only through the epoch-92 boundary at 39,744,000.
// The final 50-slot tail after the last observed v1.2 marker remains
// candidate-only and requires external checkpoint qualification.
// Compatibility must be demonstrated by trusted checkpoints before the
// parent may route or publish this candidate. Structural snapshot decoding is
// not treated as evidence that v1.2.32 execution semantics apply out of era.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 26_351_963;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 39_744_000;
    ...
    pub fn initialize(
        ...
        let (mut bank, source) = match initial_state {
            jetstreamer_historical_protocol::InitialState::SnapshotArchive { archive_path } => {
                ...
                let loaded = snapshot::load_archive(archive_path_ref, &state_dir, &genesis)?;
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.2.32 candidate requires a snapshot in {}..{}",
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.2.32 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.2.32 candidate entry slot {} is outside {}..{}",
...
fn restore_mainnet_runtime_hooks(bank: &mut Bank, genesis: &GenesisConfig) -> Result<(), String> {
    ...
    let bpf_loader_account = bank
        ...
        .ok_or_else(|| {
            "mainnet snapshot is missing the activated BPF loader account".to_string()
        })?;
    if bpf_loader_account.owner != native_loader::id()
        || !bpf_loader_account.executable
        || bpf_loader_account.data != b"solana_bpf_loader_program"
    {
        return Err("mainnet snapshot has an invalid activated BPF loader account".to_string());
    }
    ...
    // Exact Stable callback semantics in this era have no inflation/program
    // activations, but disable CPI before epoch 63 and enable it from epoch 63.
    // `MessageProcessor::is_cross_program_supported` is serde-skipped and its
    // default is `true`, so a restored epoch-61/62 snapshot must be corrected
    ...
    bank.set_entered_epoch_callback(Box::new(|bank| {
        assert!(
            ...
            "Solana v1.2.32 candidate entered unsupported epoch {}",
            bank.epoch()
        );
        bank.set_cross_program_support(bank.epoch() >= 63);
    }));
...
mod tests {
    ...
    fn candidate_range_is_closed_at_both_ends() {
        ...
        assert!(validate_candidate_snapshot_slot(39_743_950).is_ok());
        ...
        assert!(validate_candidate_entry_slot(39_743_950).is_ok());
    ...
    #[test]
    fn epoch_61_allows_only_vote_transactions_and_epoch_62_resumes_normal_traffic() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
        ...
        let epoch_61_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(61);
        let bank61 = Arc::new(Bank::new_from_parent(
            &Arc::new(bank0),
            &leader,
            epoch_61_slot,
        ));
        ...
        assert_eq!(
            bank61.transfer(1, &genesis.mint_keypair, &recipient),
            Err(TransactionError::ClusterMaintenance)
        );

        let epoch_62_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(62);
        let bank62 = Bank::new_from_parent(&bank61, &leader, epoch_62_slot);
        assert!(bank62
            .transfer(1, &genesis.mint_keypair, &recipient)
            .is_ok());
    }

    #[test]
    fn restored_epoch_62_disables_cpi_until_epoch_63() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
        set_exact_mainnet_native_programs(&mut genesis.genesis_config);
        let state_dir = snapshot::private_state_dir(None).unwrap();
        ...
        // corrects it immediately at epoch 62 and reenables it at epoch 63.
        let epoch_62_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(62);
        let mut bank62 = Bank::new_from_parent(&Arc::new(bank0), &leader, epoch_62_slot);
        assert!(bank62.cross_program_support());
        restore_mainnet_runtime_hooks(&mut bank62, &genesis.genesis_config).unwrap();
        assert!(!bank62.cross_program_support());

        let epoch_63_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(63);
        let bank63 = Bank::new_from_parent(&Arc::new(bank62), &leader, epoch_63_slot);
        assert!(bank63.cross_program_support());
    }
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_2_32_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_26351963")
            .expect("set JETSTREAMER_SNAPSHOT_26351963");
        let ledger =
