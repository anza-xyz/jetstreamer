...
/// An owned copy of one physical AppendVec write. This compatibility-only
/// view is ordered by the persisted global write version; it does not affect
/// the account index or hashing behavior.
#[derive(Clone, Debug)]
pub struct OwnedAccountWrite {
    pub slot: Slot,
...
impl AccountsDb {
        ...
        B: Send + Default + Sync,
    {
        if let Some(slot_cache) = self.accounts_cache.slot_cache(slot) {
            ...
            let storage_maps: Vec<Arc<AccountStorageEntry>> = self
                ...
                .unwrap_or_default();
            // Entering the old Rayon pool for one AppendVec wakes every worker
            // but cannot expose any parallelism. Historical replay hits this
            // path for nearly every small per-slot delta.
            if storage_maps.len() <= 1 {
                ...
                return ScanStorageResult::Stored(retval);
            }
    ...
    /// Return a stable, owned view of every physical account write at or after
    /// `first_write_version`, bounded by the next version observed when this
    /// method starts. Sorting after the scan removes AppendVec shard order as
    /// an observable detail. Historical workers disable the v1.6 account cache
    /// so every replay write retains its persisted global version.
    pub fn ordered_account_writes_since(
        &self,
        slot: Slot,
        first_write_version: u64,
    ) -> (u64, Vec<OwnedAccountWrite>) {
        let next_write_version = self.write_version.load(Ordering::Acquire);
        let mut writes = Vec::new();
        if let Some(stores) = self.storage.get_slot_storage_entries(slot) {
            for store in stores {
                for stored_account in store.all_accounts() {
                    let write_version = stored_account.meta.write_version;
                    if write_version >= first_write_version && write_version < next_write_version {
                        writes.push(OwnedAccountWrite {
                            slot,
                            write_version,
                            pubkey: stored_account.meta.pubkey,
                            account: stored_account.clone_account().into(),
                            stored_hash: *stored_account.hash,
                        });
                    }
                }
            }
        }
        writes.sort_unstable_by_key(|write| write.write_version);
        (next_write_version, writes)
    }
