...
// The epoch-147 terminal checkpoint is the bootstrap for this candidate.
// Admit entries only for epochs 148 and 149. Epoch 150 begins the separately
// checkpoint-qualified v1.5.6 envelope.
...
// is not treated as evidence that v1.5.5 execution semantics apply out of era.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 63_935_659;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 64_800_000;
    ...
    pub fn initialize(
        ...
        let (bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.5.5 candidate requires a snapshot in {}..{}",
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.5.5 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.5.5 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.5.5 ledger::entry::next_hash. Pulling in
...
/// Exact additional loader table passed by the v1.5.5 ledger restore path on
...
mod tests {
    ...
    #[test]
    fn v1_5_5_statuses_preserve_existing_wire_values() {
        assert_eq!(
    ...
    #[test]
    fn additional_loader_table_matches_upstream_v1_5_5_ledger() {
    ...
    #[test]
    fn v1_5_5_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_5_5_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_63935659")
            .expect("set JETSTREAMER_SNAPSHOT_63935659");
        let ledger =
