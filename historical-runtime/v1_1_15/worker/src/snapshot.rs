...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.1.15-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(archive_path: &Path, state_dir: &TempDir) -> Result<LoadedSnapshot, String> {
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // Exact v1.1.15 `rebuild_bank_from_snapshots` schema: snapshot 1.1.0
        ...
        let mut bank: Bank = deserialize_from_snapshot(stream.by_ref())
            .map_err(|error| format!("failed to decode v1.1.15 Bank: {}", error))?;
...
/// Extracts the narrow v1.1.15 handoff snapshot layout into worker-private
...
mod tests {
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_1_15_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_13334356")
            .expect("set JETSTREAMER_SNAPSHOT_13334356 to run this integration test");
        let state_dir = private_state_dir(None).unwrap();
        let loaded = load_archive(Path::new(&path), &state_dir).unwrap();
        assert_eq!(loaded.bank.slot(), 13_334_356);
        assert_eq!(
