...
// Mainnet restarted with the v1.2.26 CPI and vote-program hotfixes during
// epoch 67. Reconstruct both changes at the same candidate restart boundary;
// the enclosing replay must still match the independent terminal accounts
// hash before this worker is admissible.
const VOTE_TIMESTAMP_ACTIVATION_SLOT: Slot = 29_371_188;
...
impl VoteState {
    ...
    pub fn process_timestamp(
        ...
        self.process_timestamp_with_equal_timestamp(slot, timestamp, true)
    }

    fn process_timestamp_with_equal_timestamp(
        &mut self,
        slot: Slot,
        timestamp: UnixTimestamp,
        allow_equal_timestamp_on_later_slot: bool,
    ) -> Result<(), VoteError> {
        if (slot < self.last_timestamp.slot || timestamp < self.last_timestamp.timestamp)
            || ((slot == self.last_timestamp.slot
                || (!allow_equal_timestamp_on_later_slot
...
pub fn process_vote<S: std::hash::BuildHasher>(
    ...
    if let Some(timestamp) = vote.timestamp {
        let allow_equal_timestamp_on_later_slot = clock.slot >= VOTE_TIMESTAMP_ACTIVATION_SLOT;
        vote.slots
            .iter()
            .max()
            .ok_or_else(|| VoteError::EmptySlots)
            .and_then(|slot| {
                vote_state.process_timestamp_with_equal_timestamp(
                    *slot,
                    timestamp,
                    allow_equal_timestamp_on_later_slot,
                )
...
mod tests {
    ...
    #[test]
    fn test_equal_timestamp_activation_gate() {
        assert_eq!(VOTE_TIMESTAMP_ACTIVATION_SLOT, 29_371_188);
        let (slot, timestamp) = (15, 1_575_412_285);
        let mut vote_state = VoteState::default();
        vote_state.last_timestamp = BlockTimestamp { slot, timestamp };

        assert_eq!(
            vote_state.process_timestamp_with_equal_timestamp(slot + 1, timestamp, false),
            Err(VoteError::TimestampTooOld)
        );
        assert_eq!(
            vote_state.last_timestamp,
        ...
        assert_eq!(
            vote_state.process_timestamp_with_equal_timestamp(slot + 1, timestamp, true),
            Ok(())
        );
    }
