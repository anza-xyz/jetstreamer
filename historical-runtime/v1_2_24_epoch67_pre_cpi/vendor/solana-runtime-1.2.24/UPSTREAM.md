...
- Upstream `runtime/` Git tree: `f15d5477372a026c26dd203cfed525fc1f131ae9`
...
```text
git rev-parse v1.2.24^{} v1.2.24^{}:runtime v1.2.24^{}:LICENSE
```
...
- read-only `Bank::cross_program_support` exposes the serde-skipped CPI gate
  so the worker can regression-test exact snapshot restoration and
  entered-epoch transition semantics.
...
path. Static loader dispatch calls the exact linked v1.2.24 entrypoint with the
same arguments as the upstream dynamic loader. These changes do not alter
storage, account indexing, hashing, transaction execution, or serialization.
