...
/// Screens raw tar blocks before tar-rs sees them. tar 0.4.29's normal entry
...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.1.23-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(archive_path: &Path, state_dir: &TempDir) -> Result<LoadedSnapshot, String> {
    ...
    if !is_supported_snapshot_version(snapshot_version) {
        return Err(format!(
            ...
            snapshot_version, SNAPSHOT_VERSION_1_1
        ));
    }
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // Exact v1.1.23 `rebuild_bank_from_snapshots` schema: snapshot 1.1.0
        // stores Bank followed by BankRcSerialize in the same data file.
        debug_assert_eq!(snapshot_version, SNAPSHOT_VERSION_1_1);
        let mut bank: Bank = deserialize_from_snapshot(stream.by_ref())
            .map_err(|error| format!("failed to decode v1.1.23 Bank: {}", error))?;
...
fn is_supported_snapshot_version(version: &str) -> bool {
    version == SNAPSHOT_VERSION_1_1
}

/// Extracts the narrow v1.1.23 handoff snapshot layout into worker-private
...
mod tests {
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_1_23_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_25488049")
            .expect("set JETSTREAMER_SNAPSHOT_25488049 to run this integration test");
        let state_dir = private_state_dir(None).unwrap();
        let loaded = load_archive(Path::new(&path), &state_dir).unwrap();
        assert_eq!(loaded.bank.slot(), 25_488_049);
        assert_eq!(
