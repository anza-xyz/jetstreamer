...
const BACKEND_ID: &str = "solana-v1.1.23";
const SOLANA_TAG: &str = "v1.1.23";
const SOLANA_COMMIT: &str = "263fc25992ebae85e7ba2f176e9a066449489c3e";
    ...
    for write in writes {
        if write.data.len() as u64 > MAX_PERMITTED_DATA_LENGTH {
            return Err(io::Error::new(
                ...
                "historical account write exceeds v1.1.23's maximum account-data length",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn account_write_above_v1123_limit_is_rejected_before_output() {
        let mut entry = processed_entry(0);
    ...
    fn handshake_is_explicitly_unqualified_candidate() {
        ...
        assert_eq!(handshake.backend_id, "solana-v1.1.23");
        assert_eq!(
            handshake.solana_commit,
            "263fc25992ebae85e7ba2f176e9a066449489c3e"
        );
        assert_eq!(
