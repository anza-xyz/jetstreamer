...
use crate::{
    ...
    message_processor::{MessageProcessor, ProcessInstructionWithContext},
    nonce_utils,
...
// Mainnet restarted with the v1.2.26 CPI hotfix during epoch 67. This
// candidate uses the first slot after the last canonical transaction that
// proves the old, disabled behavior; the enclosing replay must still match
// the independent terminal accounts hash before the worker is admissible.
const MAINNET_CPI_ACTIVATION_SLOT: Slot = 29_371_188;
...
impl Bank {
    ...
    pub fn new_from_parent(parent: &Arc<Bank>, collector_id: &Pubkey, slot: Slot) -> Self {
        ...
        // Reconstruct mainnet's observed activation state for the sealed
        // epoch-67/68 checkpoint cohort. The snapshot does not serialize this
        // MessageProcessor flag. Canonical transactions prove that CPI was
        // still disabled through slot 29,371,187; test the first behaviorally
        // admissible child slot as the restart boundary.
        if new.slot() >= MAINNET_CPI_ACTIVATION_SLOT {
    ...
    /// Report whether cross-program invocation is enabled for this bank.
    pub fn cross_program_support(&self) -> bool {
        self.message_processor.cross_program_support()
    }

    /// Register a statically linked loader entrypoint. Historical replay uses
    /// this to restore the serde-skipped BPF loader without consulting a
    /// deployment-adjacent shared object.
    pub fn add_builtin_loader(
        ...
        self.message_processor
            .add_loader(program_id, process_instruction);
    }
