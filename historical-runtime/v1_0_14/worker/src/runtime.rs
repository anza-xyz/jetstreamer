...
/// Exact equivalent of v1.0.14 ledger::entry::next_hash. Pulling in
...
/// Restore it with the exact v1.0.14 processors linked into this worker, so
...
fn restore_mainnet_runtime_hooks(bank: &mut Bank, genesis: &GenesisConfig) -> Result<(), String> {
    ...
    bank.set_entered_epoch_callback(Box::new(|bank| {
        assert!(
            ...
            "Solana v1.0.14 candidate entered unsupported epoch {}",
            bank.epoch()
        );
    }));
    Ok(())
...
mod tests {
    ...
    #[test]
    fn v1_0_14_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_epoch_9_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_3887911")
            .expect("set JETSTREAMER_SNAPSHOT_3887911");
        let ledger =
        ...
        .unwrap();
        assert_eq!(initialized.slot, 3_887_911);
        let checkpoint = state.freeze_checkpoint(3_887_911).unwrap();
        println!(
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_epoch_10_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_4319880")
            .expect("set JETSTREAMER_SNAPSHOT_4319880");
        let ledger =
            std::env::var("JETSTREAMER_MAINNET_LEDGER").expect("set JETSTREAMER_MAINNET_LEDGER");
        let (mut state, initialized) = RuntimeState::initialize(
            &ledger,
        ...
        .unwrap();
        assert_eq!(initialized.slot, 4_319_880);
        let checkpoint = state.freeze_checkpoint(4_319_880).unwrap();
        assert_eq!(
            Hash::new(&checkpoint.accounts_hash).to_string(),
