...
/// Write the v1.2.24 `1.2.0` snapshot format without invoking a host-side
...
pub fn export_archive(
    ...
    let staging = Builder::new()
        .prefix("jetstreamer-v1.2.24-snapshot-staging-")
    ...
    let archive = Builder::new()
        .prefix("jetstreamer-v1.2.24-snapshot-archive-")
...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.2.24-epoch67-pre-cpi-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // This is the exact v1.2.24 `rebuild_bank_from_snapshots` dispatch:
        ...
        let mut bank: Bank = deserialize_from_snapshot(stream.by_ref())
            .map_err(|error| format!("failed to decode v1.2.24 Bank: {}", error))?;
...
mod tests {
    ...
    fn parses_historical_archive_identity() {
        ...
        let (_, _, compression) = parse_archive_identity(Path::new(
            "snapshot-29460107-E3ZFdfxfS3xcPi8b8sm2D9efuPNJWSPVHpDahCN6uvuS.tar.zst",
        ))
        .unwrap();
    ...
    #[test]
    fn accepts_only_the_two_upstream_v1_2_24_snapshot_schemas() {
        assert_eq!(
