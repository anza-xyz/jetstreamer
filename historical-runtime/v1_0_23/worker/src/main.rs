...
const BACKEND_ID: &str = "solana-v1.0.23";
const SOLANA_TAG: &str = "v1.0.23";
const SOLANA_COMMIT: &str = "825c0e2b6e39ae67431ed0a8282260ad3914c87a";
    ...
    for write in writes {
        if write.data.len() as u64 > MAX_PERMITTED_DATA_LENGTH {
            return Err(io::Error::new(
                ...
                "historical account write exceeds v1.0.23's maximum account-data length",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn account_write_above_v1023_limit_is_rejected_before_output() {
        let mut entry = processed_entry(0);
    ...
    fn handshake_is_explicitly_unqualified_candidate() {
        ...
        assert_eq!(handshake.backend_id, "solana-v1.0.23");
        assert_eq!(
            handshake.solana_commit,
            "825c0e2b6e39ae67431ed0a8282260ad3914c87a"
        );
        assert_eq!(
