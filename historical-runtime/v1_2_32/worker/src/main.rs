...
const BACKEND_ID: &str = "solana-v1.2.32";
const SOLANA_TAG: &str = "v1.2.32";
const SOLANA_COMMIT: &str = "8c989da68342918f1717c60aa60fdfab7d1e676e";
    ...
    for write in writes {
        if write.data.len() as u64 > MAX_PERMITTED_DATA_LENGTH {
            return Err(io::Error::new(
                ...
                "historical account write exceeds v1.2.32's maximum account-data length",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn account_write_above_v1232_limit_is_rejected_before_output() {
        let mut entry = processed_entry(0);
    ...
    fn handshake_is_explicitly_unqualified_candidate() {
        ...
        assert_eq!(handshake.backend_id, "solana-v1.2.32");
        assert_eq!(
