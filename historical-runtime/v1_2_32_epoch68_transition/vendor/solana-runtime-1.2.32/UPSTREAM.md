...
- `Bank::add_builtin_loader` restores the serde-skipped, exact static BPF
  ...
  would pass, without consulting a mutable shared object next to the deployment; and
- read-only `Bank::cross_program_support` exposes the serde-skipped CPI gate;
  and
- the v1.2.32 child-bank CPI hotfix is activated at slot `29,371,188`, the
  first behaviorally admissible child after the last canonical transaction
  proving the old behavior, rather than inferred from the snapshot's epoch.
...
same arguments as the upstream dynamic loader. Except for the explicitly
documented mainnet CPI activation reconstruction, these changes do not alter
