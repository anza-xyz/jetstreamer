...
const BACKEND_ID: &str = "solana-v1.0.18";
const SOLANA_TAG: &str = "v1.0.18";
const SOLANA_COMMIT: &str = "f26f18d29d650d06f5c5b7a4eb625622a999ea66";
    ...
    for write in writes {
        if write.data.len() as u64 > MAX_PERMITTED_DATA_LENGTH {
            return Err(io::Error::new(
                ...
                "historical account write exceeds v1.0.18's maximum account-data length",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn account_write_above_v1018_limit_is_rejected_before_output() {
        let mut entry = processed_entry(0);
    ...
    fn handshake_is_explicitly_unqualified_candidate() {
        ...
        assert_eq!(handshake.backend_id, "solana-v1.0.18");
        assert_eq!(
            handshake.solana_commit,
            "f26f18d29d650d06f5c5b7a4eb625622a999ea66"
        );
        assert_eq!(
