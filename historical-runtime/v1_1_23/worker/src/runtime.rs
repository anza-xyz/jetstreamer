...
// The v1.1.23 terminal-patch candidate is a diagnostic envelope for the v1.1
// history. It starts at the canonical v1.1.14 archive immediately before
// epoch 30 and ends at epoch 61's boundary. Every epoch remains candidate-only
// and must match all trusted snapshot checkpoints. Any mismatch fails closed
// and requires splitting the envelope around an earlier exact patch worker.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 12_959_684;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 26_352_000;
const MAX_SUPPORTED_EPOCH: u64 = 60;
...
impl RuntimeState {
    pub fn initialize(
        ...
        let (mut bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.1.23 candidate requires a snapshot in {}..{}",
    ...
    fn advance_to(&mut self, slot: u64) -> Result<Vec<AccountWrite>, String> {
        ...
        // v1.1.23 clones the exact static processor table from its parent.
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.1.23 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.1.23 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.1.23 ledger::entry::next_hash. Pulling in
...
/// Restore it with the exact v1.1.23 processors linked into this worker, so
...
fn restore_mainnet_runtime_hooks(bank: &mut Bank, genesis: &GenesisConfig) -> Result<(), String> {
    ...
    if bank.epoch_schedule() != &genesis.epoch_schedule
        || bank.epoch() != genesis.epoch_schedule.get_epoch(bank.slot())
        || bank.epoch() > MAX_SUPPORTED_EPOCH
    {
        return Err(format!(
            "snapshot slot/epoch schedule is inconsistent with the v1.1.23 candidate: slot {}, epoch {}",
            bank.slot(),
            bank.epoch()
        ));
    }
    if bank.ticks_per_slot() != genesis.ticks_per_slot
        || bank.hashes_per_tick() != &genesis.poh_config.hashes_per_tick
    {
    ...
    for (name, program_id) in &genesis.native_instruction_processors {
        validate_native_program_account(bank, name, *program_id)?;
    }
    // These serde-skipped fields are restored in the same order as upstream
    // `rebuild_bank_from_snapshots`: operating mode first, then `finish_init`
    // to install the exact v1.1.23 built-in processor table.
    bank.operating_mode = Some(genesis.operating_mode);
    bank.finish_init();
    restore_bpf_loader(bank)?;

    // Stable mainnet activates the BPF loader account at epoch 34. The exact
    // upstream callback creates that native account; bind the linked v1.1.23
    // processor at the same transition so replay never loads a neighboring
    // deployment's dynamic library. Other epochs in this envelope have no
    // callback state transition.
    bank.set_entered_epoch_callback(Box::new(|bank| {
        assert!(
            bank.epoch() <= MAX_SUPPORTED_EPOCH,
            "Solana v1.1.23 candidate entered unsupported epoch {}",
            bank.epoch()
        );
        if bank.epoch() == BPF_LOADER_ACTIVATION_EPOCH {
...
/// account, but snapshot serde intentionally omitted its in-memory function
/// pointer. Bind the exact v1.1.23 implementation statically so replay cannot
/// depend on a deployment-adjacent `libsolana_bpf_loader_program.so`.
fn restore_bpf_loader(bank: &mut Bank) -> Result<(), String> {
    let program_id = solana_bpf_loader_program::id();
    match bank.get_account(&program_id) {
        Some(_) => {
            validate_native_program_account(bank, BPF_LOADER_NAME, program_id)?;
            bank.add_static_program(
                BPF_LOADER_NAME,
                program_id,
                solana_bpf_loader_program::process_instruction,
        ...
        None if bank.epoch() < BPF_LOADER_ACTIVATION_EPOCH => Ok(()),
        None => Err(format!(
            "snapshot at epoch {} is missing activated v1.1.23 BPF loader account {}",
            bank.epoch(),
            program_id
        )),
    }
...
fn validate_native_program_account(
    bank: &Bank,
    name: &str,
    program_id: Pubkey,
) -> Result<(), String> {
    let account = bank.get_account(&program_id).ok_or_else(|| {
        format!(
            "snapshot is missing v1.1.23 native program account {} ({:?})",
            program_id, name
        )
    })?;
    if account.owner != native_loader::id()
        || !account.executable
        || account.data.as_slice() != name.as_bytes()
    {
        return Err(format!(
            "snapshot has invalid v1.1.23 native program account {} ({:?})",
            program_id, name
        ));
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn v1_1_23_candidate_range_is_closed_at_both_ends() {
    ...
    #[test]
    fn v1_1_23_error_vocabulary_is_lossless() {
        assert_eq!(
            normalize_transaction_error(TransactionError::ClusterMaintenance),
            WireTransactionError::ClusterMaintenance
        );
        let cases = [
            (
                InstructionError::ExecutableDataModified,
                WireInstructionError::ExecutableDataModified,
            ),
            (
                InstructionError::ExecutableLamportChange,
                WireInstructionError::ExecutableLamportChange,
            ),
            (
                InstructionError::ExecutableAccountNotRentExempt,
                WireInstructionError::ExecutableAccountNotRentExempt,
        ...
        for (upstream, wire) in cases.iter() {
            assert_eq!(normalize_instruction_error(upstream.clone()), wire.clone());
        }
    }
    ...
    #[test]
    fn bpf_loader_restore_rejects_missing_or_malformed_activation_state() {
        let leader = Pubkey::new_from_array([7; 32]);
        ...
        let epoch_34_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(34);
        let mut missing = Bank::new_from_parent(&bank0, &leader, epoch_34_slot);
        assert!(restore_bpf_loader(&mut missing)
            .unwrap_err()
            .contains("missing activated"));

        let mut malformed = Account::new(1, 0, &native_loader::id());
        malformed.data = b"not_the_bpf_loader".to_vec();
        malformed.executable = true;
        missing.store_account(&solana_bpf_loader_program::id(), &malformed);
        assert!(restore_bpf_loader(&mut missing)
            .unwrap_err()
            .contains("invalid v1.1.23 native program account"));
    }

    #[test]
    fn epoch_34_callback_activates_and_binds_the_exact_bpf_loader() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
        ...
        let epoch_34_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(BPF_LOADER_ACTIVATION_EPOCH);
        let bank = Bank::new_from_parent(&bank0, &leader, epoch_34_slot);

        validate_native_program_account(&bank, BPF_LOADER_NAME, solana_bpf_loader_program::id())
            .unwrap();
        let transaction = Transaction::new_signed_with_payer(
            vec![Instruction {
                program_id: solana_bpf_loader_program::id(),
                accounts: Vec::new(),
                data: Vec::new(),
            }],
    ...
    #[test]
    fn epoch_40_switches_from_legacy_to_current_system_processor() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
        set_exact_mainnet_native_programs(&mut genesis.genesis_config);
        let state_dir = snapshot::private_state_dir(None).unwrap();
        ...
        // Cross epoch 34 so this follows the same callback history as mainnet,
        // then approach the system-program activation boundary from epoch 39.
        let epoch_34_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(BPF_LOADER_ACTIVATION_EPOCH);
        let bank34 = Arc::new(Bank::new_from_parent(
            &Arc::new(bank0),
            &leader,
            epoch_34_slot,
        ));
        let epoch_39_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(39);
        let bank39 = Arc::new(Bank::new_from_parent(&bank34, &leader, epoch_39_slot));

        // The legacy processor rejects self-transfer; the epoch-40 processor
        // accepts it. This distinguishes the actual linked dispatch function,
        // not merely the epoch number or persisted system-program account.
        assert!(bank39
            .transfer(1, &genesis.mint_keypair, &genesis.mint_keypair.pubkey(),)
            .is_err());
        let epoch_40_slot = genesis
            .genesis_config
            .epoch_schedule
            .get_first_slot_in_epoch(40);
        let bank40 = Bank::new_from_parent(&bank39, &leader, epoch_40_slot);
        assert!(bank40
            .transfer(2, &genesis.mint_keypair, &genesis.mint_keypair.pubkey(),)
            .is_ok());
    }

    #[test]
    fn v1_1_23_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
        set_exact_mainnet_native_programs(&mut genesis.genesis_config);
        let state_dir = snapshot::private_state_dir(None).unwrap();
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_1_23_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_25488049")
            .expect("set JETSTREAMER_SNAPSHOT_25488049");
        let ledger =
        ...
        .unwrap();
        assert_eq!(initialized.slot, 25_488_049);
        let checkpoint = state.freeze_checkpoint(25_488_049).unwrap();
        println!(
        ...
        assert_eq!(
            ...
            "6Mj2yawDYC8Lz59d8vxpCAP7K99Uf9sDUFrdgodEMZUz"
        );
    }
}
