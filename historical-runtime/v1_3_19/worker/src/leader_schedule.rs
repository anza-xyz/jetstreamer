//! Exact v1.3.19 stake-to-leader algorithm, copied in small form from
...
pub fn schedule_for_epoch(bank: &Bank, epoch: Epoch) -> Result<Vec<Pubkey>, String> {
    ...
    // This ordering is consensus-sensitive and matches v1.3.19 exactly.
...
pub fn slot_leader(
    ...
    if let Entry::Vacant(entry) = schedules.entry(epoch) {
        entry.insert(schedule_for_epoch(bank, epoch)?);
    }
