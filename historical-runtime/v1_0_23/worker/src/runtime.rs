...
// patch worker before publication can continue.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 5_183_736;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 12_960_000;
const MAX_SUPPORTED_EPOCH: u64 = 29;
    ...
    pub fn initialize(
        ...
        let (mut bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.0.23 epochs 12-29 candidate requires a snapshot in {}..{}",
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.0.23 epochs 12-29 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.0.23 epochs 12-29 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.0.23 ledger::entry::next_hash. Pulling in
...
/// Restore it with the exact v1.0.23 processors linked into this worker, so
...
fn restore_mainnet_runtime_hooks(bank: &mut Bank, genesis: &GenesisConfig) -> Result<(), String> {
    ...
    // The exact upstream Stable callback has no state effects in epochs 12-29.
    ...
    bank.set_entered_epoch_callback(Box::new(|bank| {
        assert!(
            ...
            "Solana v1.0.23 candidate entered unsupported epoch {}",
            bank.epoch()
        );
    }));
    Ok(())
...
mod tests {
    ...
    #[test]
    fn v1_0_23_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_0_23_boundary_snapshot() {
        const SNAPSHOT_SLOT: u64 = 12_095_792;
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_12095792")
            .expect("set JETSTREAMER_SNAPSHOT_12095792");
        let ledger =
