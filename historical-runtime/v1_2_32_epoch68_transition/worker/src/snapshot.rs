...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.2.32-epoch68-transition-");
    match scratch_root {
        Some(root) => {
...
mod tests {
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_2_32_epoch68_transition_snapshot() {
        let path = std::env::var("JETSTREAMER_V1_2_32_EPOCH68_TRANSITION_SNAPSHOT")
            .expect("set JETSTREAMER_V1_2_32_EPOCH68_TRANSITION_SNAPSHOT to run this test");
        ...
        assert_eq!(loaded.bank.slot(), 28_943_975);
