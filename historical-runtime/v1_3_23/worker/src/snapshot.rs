...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.3.23-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // Exact v1.3.23 `bank_from_archive` schema. The upstream reconstruction
        ...
        .map_err(|error| format!("failed to decode v1.3.23 snapshot bank: {}", error))?;
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_upstream_v1_3_23_snapshot_schema() {
        assert_eq!(
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_3_23_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_43631879")
            .expect("set JETSTREAMER_SNAPSHOT_43631879 to run this integration test");
        let ledger =
        ...
        assert_eq!(loaded.bank.slot(), 43_631_879);
