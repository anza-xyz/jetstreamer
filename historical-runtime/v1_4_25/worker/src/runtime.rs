...
// checkpoint-gated v1.5 candidate envelope.
...
// is not treated as evidence that v1.4.25 execution semantics apply out of era.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 51_407_922;
    ...
    pub fn initialize(
        ...
        let (bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.4.25 candidate requires a snapshot in {}..{}",
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.4.25 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.4.25 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.4.25 ledger::entry::next_hash. Pulling in
...
/// Exact additional loader table passed by the v1.4.25 ledger restore path on
...
mod tests {
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_4_25_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_51407922")
            .expect("set JETSTREAMER_SNAPSHOT_51407922");
        let ledger =
