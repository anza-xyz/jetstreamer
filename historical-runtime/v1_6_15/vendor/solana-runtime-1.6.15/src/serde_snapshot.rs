use {
    ...
    bincode,
    bincode::{config::Options, BincodeRead, Error},
    log::*,
...
const MAX_STREAM_SIZE: u64 = MAX_SNAPSHOT_DATA_FILE_SIZE;
/// Hard bound for one length-prefixed bytes/String value while decoding a
/// snapshot. The aggregate stream limit alone does not stop bincode from
/// reserving an attacker-declared field before it reaches EOF.
pub const MAX_SNAPSHOT_VARIABLE_FIELD_SIZE: usize = 16 * 1024 * 1024;

struct SnapshotBincodeReader<R> {
    inner: R,
    buffer: Vec<u8>,
}

impl<R> SnapshotBincodeReader<R> {
    fn new(inner: R) -> Self {
        Self {
            inner,
            buffer: Vec::new(),
    ...
    fn check_variable_length(length: usize) -> bincode::Result<()> {
        if length > MAX_SNAPSHOT_VARIABLE_FIELD_SIZE {
    ...
    fn fill_buffer(&mut self, length: usize) -> bincode::Result<()>
    where
        R: Read,
    {
        Self::check_variable_length(length)?;
...
impl<R: Read> Read for SnapshotBincodeReader<R> {
...
impl<'a, R: Read> BincodeRead<'a> for SnapshotBincodeReader<R> {
        ...
        V: serde::de::Visitor<'a>,
    {
        ...
        let value = std::str::from_utf8(&self.buffer)
            .map_err(|error| Box::new(bincode::ErrorKind::InvalidUtf8Encoding(error)))?;
    ...
    T: DeserializeOwned,
{
    bincode::options()
        ...
        .deserialize_from_custom(SnapshotBincodeReader::new(reader))
}

/// Decode with the exact snapshot bincode options and the per-field
/// allocation bound. Exposed narrowly for the isolated worker's status-cache
/// decoder and security tests.
...
fn reconstruct_accountsdb_from_fields<E>(
    accounts_db_fields: AccountsDbFields<E>,
    account_paths: &[PathBuf],
    mut unpacked_append_vec_map: UnpackedAppendVecMap,
    ...
    E: SerializableStorage,
{
    ...
    let mut storage = storage
        ...
        .map(|(slot, mut slot_storage)| {
            ...
            for storage_entry in slot_storage.drain(..) {
                ...
                let append_vec_path =
                    unpacked_append_vec_map.remove(&file_name).ok_or_else(|| {
                        io::Error::new(
    ...
    if let Some(file_name) = unpacked_append_vec_map.keys().min() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("unreferenced AppendVec in snapshot: {}", file_name),
        )
        .into());
    }
