...
pub mod tests {
    ...
    #[test]
    fn test_scan_account_storage_single_store_and_ordered_writes() {
        let db = AccountsDB::new(Vec::new(), OperatingMode::Development);
        ...
        let first = db.next_write_version();
        ...
        let (next, writes) = db.ordered_account_writes_since(7, first);
        assert_eq!(next, first + 2);
        assert_eq!(writes.len(), 2);
        assert_eq!(writes[0].write_version, first);
        assert_eq!(writes[1].write_version, first + 1);
    }
