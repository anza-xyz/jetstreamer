...
// v1.1.14 and v1.1.15 have identical runtime sources.  The canonical
// predecessor snapshot was produced by v1.1.14 and the epoch-30 checkpoints
// by v1.1.15, so this worker is bounded to that one behaviorally equivalent
// epoch.  Every output remains candidate-only and checkpoint-gated.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 12_959_684;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 13_392_000;
...
impl RuntimeState {
    pub fn initialize(
        ...
        let (mut bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.1.15 candidate requires a snapshot in {}..{}",
    ...
    fn advance_to(&mut self, slot: u64) -> Result<Vec<AccountWrite>, String> {
        ...
        // v1.1.15 clones the exact static processor table from its parent.
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.1.15 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.1.15 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.1.15 ledger::entry::next_hash. Pulling in
...
/// Restore it with the exact v1.1.15 processors linked into this worker, so
...
fn restore_mainnet_runtime_hooks(bank: &mut Bank, genesis: &GenesisConfig) -> Result<(), String> {
    ...
    {
        return Err(format!(
            "snapshot slot/epoch schedule is inconsistent with the v1.1.15 candidate: slot {}, epoch {}",
            bank.slot(),
            bank.epoch()
        ));
    }
    ...
    // to install the exact v1.1.15 built-in processor table.
    ...
    // upstream callback creates that native account; bind the linked v1.1.15
    ...
    bank.set_entered_epoch_callback(Box::new(|bank| {
        assert!(
            bank.epoch() <= MAX_SUPPORTED_EPOCH,
            "Solana v1.1.15 candidate entered unsupported epoch {}",
            bank.epoch()
        );
...
/// pointer. Bind the exact v1.1.15 implementation statically so replay cannot
...
fn restore_bpf_loader(bank: &mut Bank) -> Result<(), String> {
    ...
    match bank.get_account(&program_id) {
        ...
        None => Err(format!(
            "snapshot at epoch {} is missing activated v1.1.15 BPF loader account {}",
            bank.epoch(),
            program_id
        )),
    }
...
fn validate_native_program_account(
    ...
    let account = bank.get_account(&program_id).ok_or_else(|| {
        format!(
            "snapshot is missing v1.1.15 native program account {} ({:?})",
            program_id, name
        )
    })?;
    ...
    {
        return Err(format!(
            "snapshot has invalid v1.1.15 native program account {} ({:?})",
            program_id, name
        ));
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn v1_1_15_candidate_range_is_closed_at_both_ends() {
    ...
    #[test]
    fn v1_1_15_error_vocabulary_is_lossless() {
        let cases = [
            (
    ...
    #[test]
    fn v1_1_15_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_1_15_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_13334356")
            .expect("set JETSTREAMER_SNAPSHOT_13334356");
        let ledger =
        ...
        assert_eq!(
            ...
            "6k2JUHXPhvnTSQf5qQ4w284xhC31zKzCMiMSMGUHo14B"
        );
    }
}
