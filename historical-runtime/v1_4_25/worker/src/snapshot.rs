...
// The audited boundary snapshots contain 144,901 members at slot 51,407,922
// and 380,435 members at slot 63,935,659, almost entirely AppendVecs. The next
// power of two admits the terminal snapshot with about 38% headroom while
...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.4.25-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // Exact v1.4.25 `bank_from_archive` schema. The upstream reconstruction
        ...
        .map_err(|error| format!("failed to decode v1.4.25 snapshot bank: {}", error))?;
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_upstream_v1_4_25_snapshot_schema() {
        assert_eq!(
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_4_25_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_51407922")
            .expect("set JETSTREAMER_SNAPSHOT_51407922 to run this integration test");
        let ledger =
        ...
        assert_eq!(loaded.bank.slot(), 51_407_922);
