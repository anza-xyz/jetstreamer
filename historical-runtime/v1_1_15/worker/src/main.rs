...
const BACKEND_ID: &str = "solana-v1.1.15";
const SOLANA_TAG: &str = "v1.1.15";
const SOLANA_COMMIT: &str = "2cdd3f835f00ca531af7141459d657f0ea60a946";
    ...
    for write in writes {
        if write.data.len() as u64 > MAX_PERMITTED_DATA_LENGTH {
            return Err(io::Error::new(
                ...
                "historical account write exceeds v1.1.15's maximum account-data length",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn account_write_above_v1115_limit_is_rejected_before_output() {
        let mut entry = processed_entry(0);
    ...
    fn handshake_is_explicitly_unqualified_candidate() {
        ...
        assert_eq!(handshake.backend_id, "solana-v1.1.15");
        assert_eq!(
            handshake.solana_commit,
            "2cdd3f835f00ca531af7141459d657f0ea60a946"
        );
        assert_eq!(
