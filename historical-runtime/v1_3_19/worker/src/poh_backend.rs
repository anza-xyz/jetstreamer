//! Fast, exact v1.3.19 Proof-of-History hashing.
//!
...
//!
//! Solana v1.3.19 retained v1.0.7's `entry::next_hash` wire semantics, so this
...
/// Exact equivalent of v1.3.19 `ledger::entry::next_hash` after the caller has
...
#[cfg(all(target_arch = "x86_64", not(feature = "poh-force-soft")))]
// Every flagged cast feeds an explicitly unaligned load/store intrinsic. No
// pointer is dereferenced with Rust's stricter `__m128i` alignment.
