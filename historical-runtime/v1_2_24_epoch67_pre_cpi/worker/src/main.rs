...
const BACKEND_ID: &str = "solana-v1.2.24-mainnet-epoch67-pre-cpi";
const SOLANA_TAG: &str = "v1.2.24";
const SOLANA_COMMIT: &str = "14bc62398944d6270698506d468cbeddc8513ccd";
    ...
    for write in writes {
        if write.data.len() as u64 > MAX_PERMITTED_DATA_LENGTH {
            return Err(io::Error::new(
                ...
                "historical account write exceeds v1.2.24's maximum account-data length",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn account_write_above_historical_limit_is_rejected_before_output() {
        let mut entry = processed_entry(0);
    ...
    fn handshake_is_explicitly_unqualified_candidate() {
        ...
        assert_eq!(
            ...
            "solana-v1.2.24-mainnet-epoch67-pre-cpi"
        );
        assert_eq!(
