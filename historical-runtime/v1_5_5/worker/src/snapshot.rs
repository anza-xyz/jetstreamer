...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.5.5-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // Exact v1.5.5 `bank_from_archive` schema. The upstream reconstruction
        ...
        .map_err(|error| format!("failed to decode v1.5.5 snapshot bank: {}", error))?;
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_upstream_v1_5_5_snapshot_schema() {
        assert_eq!(
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_5_5_boundary_snapshot() {
