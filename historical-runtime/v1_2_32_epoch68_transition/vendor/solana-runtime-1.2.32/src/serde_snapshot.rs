use {
    ...
    bincode::{deserialize_from, serialize_into, BincodeRead, DefaultOptions, Options},
...
const MAX_ACCOUNTS_DB_STREAM_SIZE: u64 = MAX_SNAPSHOT_DATA_FILE_SIZE;
...
impl<R: Read> Read for SnapshotBincodeReader<R> {
    ...
    fn read_exact(&mut self, output: &mut [u8]) -> std::io::Result<()> {
...
{
    deserialize_from_snapshot_with_limit(reader, MAX_SNAPSHOT_DATA_FILE_SIZE)
}

fn deserialize_from_snapshot_with_limit<R, T>(reader: R, limit: u64) -> bincode::Result<T>
where
    R: Read,
    T: serde::de::DeserializeOwned,
{
