...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.0.23-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(archive_path: &Path, state_dir: &TempDir) -> Result<LoadedSnapshot, String> {
    ...
    let snapshot_version = supported_snapshot_version(&version)?;
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // This is the exact v1.0.23 `rebuild_bank_from_snapshots` schema
        ...
        let mut bank: Bank = match snapshot_version {
            ...
            SNAPSHOT_VERSION_1_1 => deserialize_from_snapshot(stream.by_ref())
                .map_err(|error| format!("failed to decode v1.0.23 Bank: {}", error))?,
            _ => unreachable!(),
        };
...
fn supported_snapshot_version(version: &str) -> Result<&str, String> {
    let version = version.trim();
    if version == SNAPSHOT_VERSION_1_0 || version == SNAPSHOT_VERSION_1_1 {
        Ok(version)
    } else {
        Err(format!(
            "unsupported snapshot version {:?}; expected {:?} or {:?}",
            version, SNAPSHOT_VERSION_1_0, SNAPSHOT_VERSION_1_1
        ))
    }
}
...
pub(crate) fn parse_archive_identity(path: &Path) -> Result<(u64, Hash), String> {
    let name = path
        .file_name()
        .and_then(|name| name.to_str())
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_two_upstream_v1_0_23_snapshot_schemas() {
        assert_eq!(supported_snapshot_version("1.0.0").unwrap(), "1.0.0");
        assert_eq!(supported_snapshot_version("1.1.0\n").unwrap(), "1.1.0");
        assert!(supported_snapshot_version("1.2.0").is_err());
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_0_23_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_12095792")
            .expect("set JETSTREAMER_SNAPSHOT_12095792 to run this integration test");
        let state_dir = private_state_dir(None).unwrap();
        let loaded = load_archive(Path::new(&path), &state_dir).unwrap();
        assert_eq!(loaded.bank.slot(), 12_095_792);
        assert!(loaded.bank.verify_snapshot_bank());
    }

    #[test]
    #[ignore]
    fn loads_and_verifies_epoch_12_bootstrap_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_5183736")
            .expect("set JETSTREAMER_SNAPSHOT_5183736 to run this integration test");
        let state_dir = private_state_dir(None).unwrap();
        let loaded = load_archive(Path::new(&path), &state_dir).unwrap();
        assert_eq!(loaded.bank.slot(), 5_183_736);
        assert_eq!(
