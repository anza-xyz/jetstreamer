...
impl AccountStorageEntry {
    ...
    ///
    /// The bytes in `0..written_len` contain the serialized accounts while
    /// `file_len` is the logical file length mapped by the runtime.  Snapshot
    /// archives must preserve both values: truncating to `written_len` changes
    /// the restored storage capacity, while storing all `file_len` bytes turns
    /// the deliberately sparse tail into real archive data.
...
impl AccountsDB {
    ...
    /// an observable detail.
    pub fn ordered_account_writes_since(
        &self,
        slot: Slot,
        first_write_version: u64,
    ) -> (u64, Vec<OwnedAccountWrite>) {
        let next_write_version = self.write_version.load(Ordering::Acquire) as u64;
        let storage = self.storage.read().unwrap();
        let mut writes = Vec::new();
        if let Some(stores) = storage.0.get(&slot) {
            for store in stores.values() {
                for stored_account in store.accounts.accounts(0) {
                    ...
                    if write_version >= first_write_version && write_version < next_write_version {
                        writes.push(OwnedAccountWrite {
                            ...
                            account: stored_account.clone_account(),
    ...
    pub fn next_write_version(&self) -> u64 {
        self.write_version.load(Ordering::Acquire) as u64
    }
