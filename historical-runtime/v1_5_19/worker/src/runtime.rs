...
// Admit entries only for epochs 148 through 173; epoch 174 is the first
// checkpoint-gated v1.6 candidate envelope.
...
// is not treated as evidence that v1.5.19 execution semantics apply out of era.
    ...
    pub fn initialize(
        ...
        let (bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.5.19 candidate requires a snapshot in {}..{}",
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.5.19 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.5.19 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.5.19 ledger::entry::next_hash. Pulling in
...
/// Exact additional loader table passed by the v1.5.19 ledger restore path on
...
mod tests {
    ...
    #[test]
    fn v1_5_status_changes_normalize_without_changing_existing_wire_values() {
        assert_eq!(
    ...
    #[test]
    fn v1_5_19_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_5_19_boundary_snapshot() {
