...
// The exact v1.0.13 ledger-tool snapshot at slot 3,455,940 bootstraps the
// final 59-slot bridge into epoch 8. Admit only that bridge and epoch 8;
// later epochs use their own exact, independently checkpointed workers.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 3_455_940;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 3_888_000;
const MAX_SUPPORTED_EPOCH: u64 = 8;
    ...
    pub fn initialize(
        ...
        let (mut bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.0.13 epochs 8-11 candidate requires a snapshot at or after slot {}",
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.0.13 epochs 8-11 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.0.13 epochs 8-11 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.0.13 ledger::entry::next_hash. Pulling in
...
/// Restore it with the exact v1.0.13 processors linked into this worker, so
...
fn restore_mainnet_runtime_hooks(bank: &mut Bank, genesis: &GenesisConfig) -> Result<(), String> {
    ...
    // The upstream Stable callback has no state effects through epoch 8.
    ...
    bank.set_entered_epoch_callback(Box::new(|bank| {
        assert!(
            ...
            "Solana v1.0.13 candidate entered unsupported epoch {}",
            bank.epoch()
        );
    }));
    Ok(())
...
mod tests {
    ...
    #[test]
    fn v1_0_13_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_0_13_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_V1_0_13_BOUNDARY_SNAPSHOT")
            .expect("set JETSTREAMER_V1_0_13_BOUNDARY_SNAPSHOT");
        let ledger =
