...
impl RuntimeState {
    ...
    pub fn process_entry(&mut self, request: EntryRequest) -> Result<EntryProcessed, String> {
        if request.slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
            return Err(format!(
                "Solana v1.0.8 candidate runtime ends before slot {} (requested {})",
        ...
        // Match the v1.0.8 BlockstoreProcessor checks before changing Bank
...
/// Exact scalar equivalent of v1.0.8 ledger::entry::next_hash.  Pulling in
...
/// Restore it with the exact v1.0.8 processors linked into this worker, so
...
fn restore_mainnet_runtime_hooks(bank: &mut Bank, genesis: &GenesisConfig) -> Result<(), String> {
    ...
    bank.set_entered_epoch_callback(Box::new(|bank| {
        assert!(
            ...
            "Solana v1.0.8 candidate entered unsupported epoch {}",
            bank.epoch()
        );
    }));
    Ok(())
...
mod tests {
    ...
    #[test]
    fn v1_0_8_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
