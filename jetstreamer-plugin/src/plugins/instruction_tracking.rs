...
const OPTIMIZE_INTERVAL_SLOTS: u64 = 250;
...
impl Plugin for InstructionTrackingPlugin {
    ...
    fn on_block(
        ...
        async move {
            ...
            if let (Some(db_client), Some(rows)) = (db.as_ref(), flush_rows)
                && let Err(err) = write_instruction_events(Arc::clone(db_client), &rows).await
            {
                Self::with_thread_data(thread_id, |data| {
            ...
            if let (Some(db_client), true) = (db.as_ref(), should_optimize)
                && let Err(err) = optimize_slot_instructions(Arc::clone(db_client)).await
            {
                log::warn!(
    ...
    fn on_exit(&self, db: Option<Arc<Client>>) -> PluginFuture<'_> {
        async move {
            ...
            if let Some(db_client) = db
                && let Err(err) = optimize_slot_instructions(db_client).await
            {
