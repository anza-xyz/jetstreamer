...
const SNAPSHOT_VERSION_1_0: &str = "1.0.0";
...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.0.14-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(archive_path: &Path, state_dir: &TempDir) -> Result<LoadedSnapshot, String> {
    ...
    unpack_handoff_snapshot(decoder, &unpacked, expected_slot, HANDOFF_EXTRACTION_LIMITS).map_err(
        |error| {
            format!(
    ...
    let snapshot_version = version.trim();
    if !is_supported_snapshot_version(snapshot_version) {
        return Err(format!(
            "unsupported snapshot version {:?}; expected {:?} or {:?}",
            snapshot_version, SNAPSHOT_VERSION_1_0, SNAPSHOT_VERSION_1_1
        ));
    }
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // This is the exact v1.0.14 `rebuild_bank_from_snapshots` schema
        // dispatch: 1.0.0 stores Bank1_0 and converts it, while 1.1.0 stores
        // the current Bank. Both are followed by BankRcSerialize.
        let mut bank: Bank = match snapshot_version {
            SNAPSHOT_VERSION_1_0 => {
                ...
                old.convert_to_current()
            }
            SNAPSHOT_VERSION_1_1 => deserialize_from_snapshot(stream.by_ref())
                .map_err(|error| format!("failed to decode v1.0.14 Bank: {}", error))?,
            _ => unreachable!(),
        };
...
/// Read the filename-bound slot without opening or extracting the archive.
/// The loader independently checks this value against the decoded bank.
pub fn archive_slot(path: &Path) -> Result<u64, String> {
    parse_archive_identity(path).map(|(slot, _hash)| slot)
}

fn is_supported_snapshot_version(version: &str) -> bool {
    version == SNAPSHOT_VERSION_1_0 || version == SNAPSHOT_VERSION_1_1
}

/// Extracts the narrow v1.0.7 handoff snapshot layout into worker-private
...
mod tests {
    ...
    #[test]
    fn accepts_exact_upstream_snapshot_versions_only() {
        assert!(is_supported_snapshot_version("1.0.0"));
        assert!(is_supported_snapshot_version("1.1.0"));
        assert!(!is_supported_snapshot_version("1.0"));
        assert!(!is_supported_snapshot_version("1.1.1"));
        assert!(!is_supported_snapshot_version(" 1.1.0"));
    }

    #[test]
    fn snapshot_decoder_rejects_oversized_string_before_reading_payload() {
        let declared_length = (solana_runtime::bank::MAX_SNAPSHOT_VARIABLE_FIELD_SIZE as u64) + 1;
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_epoch_9_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_3887911")
            .expect("set JETSTREAMER_SNAPSHOT_3887911 to run this integration test");
        let state_dir = private_state_dir(None).unwrap();
        let loaded = load_archive(Path::new(&path), &state_dir).unwrap();
        assert_eq!(loaded.bank.slot(), 3_887_911);
        assert_eq!(
            loaded.expected_accounts_hash.to_string(),
            "BsYGwLpE5NBbehe1utx657A1b9vsT4nJ9scKbff1awbN"
        );
    }

    #[test]
    #[ignore]
    fn loads_and_verifies_epoch_10_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_4319880")
            .expect("set JETSTREAMER_SNAPSHOT_4319880 to run this integration test");
        let state_dir = private_state_dir(None).unwrap();
        let loaded = load_archive(Path::new(&path), &state_dir).unwrap();
        assert_eq!(loaded.bank.slot(), 4_319_880);
        assert_eq!(
            loaded.expected_accounts_hash.to_string(),
    ...
    fn loads_and_verifies_v1_0_7_export() {
        ...
        let (expected_slot, expected_accounts_hash) =
            parse_archive_identity(Path::new(&path)).unwrap();
        let state_dir = private_state_dir(None).unwrap();
        let loaded = load_archive(Path::new(&path), &state_dir).unwrap();
        assert_eq!(loaded.bank.slot(), expected_slot);
        assert_eq!(loaded.expected_accounts_hash, expected_accounts_hash);
        assert!(loaded.bank.verify_snapshot_bank());
        let roots = loaded.bank.src.roots();
        assert!(roots.contains(&expected_slot));
        assert!(loaded
            .bank
            .src
            .slot_deltas(&roots)
            .iter()
            .any(|(_, _, statuses)| !statuses.lock().unwrap().is_empty()));
    }
