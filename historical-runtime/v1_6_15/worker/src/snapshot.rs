...
// The audited boundary snapshot contains 407,245 members at slot 75,167,662,
// almost entirely AppendVecs. The next power of two admits it with about 29%
// headroom while retaining a hard inode and tar-framing amplification ceiling.
...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.6.15-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(
    ...
    let unpacked_append_vec_map = collect_unpacked_append_vecs(&append_vecs_path)?;
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // Exact v1.6.15 `bank_from_archive` schema. The upstream reconstruction
        ...
        .map_err(|error| format!("failed to decode v1.6.15 snapshot bank: {}", error))?;
        Ok(bank)
    })?;
...
fn collect_unpacked_append_vecs(path: &Path) -> Result<UnpackedAppendVecMap, String> {
    ...
    let mut append_vecs = UnpackedAppendVecMap::new();
    for entry in entries {
        ...
        if !file_type.is_file() {
            return Err(format!(
                "snapshot account entry is not a regular file: {}",
                entry.path().display()
            ));
        }
        let file_name = entry
            .file_name()
            .into_string()
            .map_err(|_| "snapshot AppendVec filename is not UTF-8".to_string())?;
        if !is_append_vec_name(&file_name) {
            return Err(format!(
                "invalid snapshot AppendVec filename: {}",
                file_name
            ));
        }
        if append_vecs
            .insert(file_name.clone(), entry.path())
            .is_some()
        {
            return Err(format!("duplicate snapshot AppendVec: {}", file_name));
        }
    }
    Ok(append_vecs)
}
...
fn deserialize_exact_file<T, F>(path: &Path, decode: F) -> Result<T, String>
where
    F: FnOnce(&mut BufReader<File>) -> Result<T, String>,
{
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_upstream_v1_6_15_snapshot_schema() {
        assert_eq!(
    ...
    #[test]
    fn snapshot_loader_indexes_only_regular_append_vecs() {
        let accounts = tempfile::tempdir().unwrap();
        let append_vec = accounts.path().join("999.123");
        fs::write(&append_vec, &b"append vec"[..]).unwrap();
        let indexed = collect_unpacked_append_vecs(accounts.path()).unwrap();
        assert_eq!(indexed.get("999.123"), Some(&append_vec));

        fs::write(accounts.path().join("not-an-append-vec"), &b"bad"[..]).unwrap();
        let error = collect_unpacked_append_vecs(accounts.path()).unwrap_err();
        assert!(
            error.contains("invalid snapshot AppendVec filename"),
            "{}",
            error
        );
    }
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_6_15_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_SNAPSHOT_75167662")
            .expect("set JETSTREAMER_SNAPSHOT_75167662 to run this integration test");
        let ledger =
        ...
        assert_eq!(loaded.bank.slot(), 75_167_662);
