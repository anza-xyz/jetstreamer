...
pub fn private_state_dir(scratch_root: Option<&str>) -> Result<TempDir, String> {
    ...
    builder.prefix("jetstreamer-historical-v1.0.13-");
    match scratch_root {
        Some(root) => {
...
pub fn load_archive(archive_path: &Path, state_dir: &TempDir) -> Result<LoadedSnapshot, String> {
    ...
    let bank = deserialize_exact_file(&bank_path, |stream| {
        // This is the exact v1.0.13 `rebuild_bank_from_snapshots` schema
        ...
        let mut bank: Bank = match snapshot_version {
            ...
            SNAPSHOT_VERSION_1_1 => deserialize_from_snapshot(stream.by_ref())
                .map_err(|error| format!("failed to decode v1.0.13 Bank: {}", error))?,
            _ => unreachable!(),
        };
...
mod tests {
    ...
    #[test]
    fn accepts_only_the_two_upstream_v1_0_13_snapshot_schemas() {
    ...
    #[test]
    #[ignore]
    fn loads_and_verifies_v1_0_13_boundary_snapshot() {
        let path = std::env::var("JETSTREAMER_V1_0_13_BOUNDARY_SNAPSHOT")
            .expect("set JETSTREAMER_V1_0_13_BOUNDARY_SNAPSHOT to run this integration test");
        let state_dir = private_state_dir(None).unwrap();
        let loaded = load_archive(Path::new(&path), &state_dir).unwrap();
        assert_eq!(loaded.bank.slot(), 3_455_940);
        assert_eq!(
            loaded.expected_accounts_hash.to_string(),
            "aCgGdj7tmTZQPF118GKxe7hn7yBv7MgVdn7dWL1Qcbw"
        );
    }
