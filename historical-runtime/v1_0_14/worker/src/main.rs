...
const BACKEND_ID: &str = "solana-v1.0.14";
const SOLANA_TAG: &str = "v1.0.14";
const SOLANA_COMMIT: &str = "8631be42ac29a062b5e26a85fc2f4c94af042afd";
    ...
    fn process_entry_batch<W: io::Write>(
        ...
        match result {
            ...
            Err(ProcessEntriesError::Runtime(message)) => {
                eprintln!(
                    "historical v1.0.14 worker request {} failed during runtime processing: {}",
                    request_id, message
                );
            ...
            Err(ProcessEntriesError::Emit(error)) => {
                eprintln!(
                    "historical v1.0.14 worker request {} failed while emitting its response: {}",
                    request_id, error
                );
                Err(error)
            }
...
fn run() -> io::Result<()> {
    ...
    while let Some(request) = read_frame::<Request, _>(&mut input)? {
        ...
        if shutdown {
            eprintln!(
                "historical v1.0.14 worker is closing after terminal request {}",
                request_id
            );
            break;
        }
...
fn main() {
    if let Err(error) = run() {
        eprintln!("historical v1.0.14 worker I/O loop failed: {}", error);
        std::process::exit(1);
    }
}
    ...
    for write in writes {
        if write.data.len() as u64 > MAX_PERMITTED_DATA_LENGTH {
            return Err(io::Error::new(
                ...
                "historical account write exceeds v1.0.14's maximum account-data length",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn account_write_above_v1014_limit_is_rejected_before_output() {
        let mut entry = processed_entry(0);
    ...
    fn handshake_is_explicitly_unqualified_candidate() {
        ...
        assert_eq!(handshake.backend_id, "solana-v1.0.14");
        assert_eq!(
            handshake.solana_commit,
            "8631be42ac29a062b5e26a85fc2f4c94af042afd"
        );
        assert_eq!(
