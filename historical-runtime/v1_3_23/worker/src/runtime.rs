    ...
    pub fn initialize(
        ...
        let (bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.3.23 candidate requires a snapshot in {}..{}",
        ...
        if transactions.is_empty() {
            ...
            for (index, transaction) in transactions.iter().enumerate() {
                let fee_calculator = match results.processing_results[index].1.as_ref() {
                    Some(HashAgeKind::DurableNoncePartial(_, account))
                    | Some(HashAgeKind::DurableNonceFull(_, account, _)) => {
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.3.23 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.3.23 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.3.23 ledger::entry::next_hash. Pulling in
...
/// Exact additional loader table passed by the v1.3.23 ledger restore path on
...
mod tests {
    ...
    #[test]
    fn v1_3_23_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_3_23_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_43631879")
            .expect("set JETSTREAMER_SNAPSHOT_43631879");
        let ledger =
