...
// The first observed archive in the v1.3 era is at slot 39,743,950. Admit
// snapshots and entries only through the end of epoch 100 at slot 43,632,000.
...
// not treated as evidence that v1.3.19 execution semantics apply out of era.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 39_743_950;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 43_632_000;
...
impl RuntimeState {
    pub fn initialize(
        ...
        let (bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.3.19 candidate requires a snapshot in {}..{}",
    ...
    fn commit_prevalidated_entry(
        ...
        if transactions.is_empty() {
            ...
            let (results, _balances, _inner_instructions, _log_messages) = self
                .bank
                .load_execute_and_commit_transactions(&batch, max_age, false, false, false);
            drop(batch);
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.3.19 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.3.19 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.3.19 ledger::entry::next_hash. Pulling in
...
/// Exact additional loader table passed by the v1.3.19 ledger restore path on
...
pub(crate) fn mainnet_additional_builtins() -> Builtins {
    Builtins {
        genesis_builtins: vec![Builtin::new(
            ...
            Entrypoint::Loader(solana_bpf_loader_program::process_instruction),
        )],
...
mod tests {
    ...
    fn test_state(ticks_per_slot: u64, hashes_per_tick: Option<u64>) -> (RuntimeState, Keypair) {
        ...
        let bank = Bank::new_with_paths(&genesis.genesis_config, account_paths, &[], None, None);
        (
    ...
    #[test]
    fn v1_3_19_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    fn entry_batch_emits_attributed_ordered_writes_and_checkpoint() {
        ...
        let state_dir = snapshot::private_state_dir(None).unwrap();
        let account_paths = snapshot::private_account_paths(&state_dir).unwrap();
        let bank0 = Bank::new_with_paths(&genesis.genesis_config, account_paths, &[], None, None);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_3_19_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_39743950")
            .expect("set JETSTREAMER_SNAPSHOT_39743950");
        let ledger =
