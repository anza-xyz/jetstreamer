...
// The epoch-149 terminal checkpoint is the bootstrap for this candidate.
// Epoch 150's first canonical vote and the trusted slot-64,807,725 checkpoint
// both require these v1.5.6 semantics. Later epochs remain checkpoint-gated;
// a narrower successor runtime can split this envelope if replay evidence
// identifies another consensus-relevant patch boundary.
...
// is not treated as evidence that v1.5.6 execution semantics apply out of era.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 64_799_804;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 75_168_000;
    ...
    pub fn initialize(
        ...
        let (bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.5.6 candidate requires a snapshot in {}..{}",
    ...
    fn commit_prevalidated_entry(
        ...
        if transactions.is_empty() {
                ...
                for (transaction, result) in transactions.iter().zip(&results.execution_results) {
                    if matches!(
                        ...
                        let slot_hashes = self
                            ...
                            .and_then(|account| from_account::<SlotHashes>(&account));
                        let vote =
                            transaction
                                .message
                                .instructions
            ...
            for (index, transaction) in transactions.iter().enumerate() {
                let fee_calculator = results.execution_results[index]
                    .1
                    .as_ref()
                ...
                outcomes.push(TransactionOutcome {
                    ...
                    error: results.execution_results[index]
                        .0
                        .clone()
                        .err()
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.5.6 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.5.6 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.5.6 ledger::entry::next_hash. Pulling in
...
/// Exact additional loader table passed by the v1.5.6 ledger restore path on
...
mod tests {
    ...
    #[test]
    fn v1_5_6_statuses_preserve_existing_wire_values() {
        assert_eq!(
            normalize_transaction_error(TransactionError::DuplicateSignature),
            WireTransactionError::DuplicateSignature
        );
        assert_eq!(
            normalize_instruction_error(InstructionError::Custom(42)),
            WireInstructionError::Custom(42)
        );
        assert_eq!(
            normalize_instruction_error(InstructionError::IncorrectAuthority),
            WireInstructionError::IncorrectAuthority
        );
    }
    ...
    #[test]
    fn additional_loader_table_matches_upstream_v1_5_6_ledger() {
    ...
    #[test]
    fn v1_5_6_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_5_6_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_64799804")
            .expect("set JETSTREAMER_SNAPSHOT_64799804");
        let ledger =
