...
const BACKEND_ID: &str = "solana-v1.0.13";
const SOLANA_TAG: &str = "v1.0.13";
const SOLANA_COMMIT: &str = "fdeda769d05fea4a3f861e787d47d995feee15d7";
    ...
    for write in writes {
        if write.data.len() as u64 > MAX_PERMITTED_DATA_LENGTH {
            return Err(io::Error::new(
                ...
                "historical account write exceeds v1.0.13's maximum account-data length",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn account_write_above_v1013_limit_is_rejected_before_output() {
        let mut entry = processed_entry(0);
    ...
    fn handshake_is_explicitly_unqualified_candidate() {
        ...
        assert_eq!(handshake.backend_id, "solana-v1.0.13");
        assert_eq!(
            handshake.solana_commit,
            "fdeda769d05fea4a3f861e787d47d995feee15d7"
        );
        assert_eq!(
