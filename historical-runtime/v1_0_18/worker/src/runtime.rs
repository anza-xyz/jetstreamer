...
// The v1.0.18 ledger-tool marker at slot 5,615,729 bootstraps the final
// 270-slot bridge into epoch 13. Admit only that bridge and entries before the
// v1.0.19 marker at slot 6,911,758. Compatibility must be demonstrated by
// trusted checkpoints before the parent may route or publish this candidate.
// Snapshot-schema compatibility alone is not evidence that v1.0.18 execution
// semantics apply outside this exact-version envelope.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 5_615_729;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 6_911_758;
const MAX_SUPPORTED_EPOCH: u64 = 15;
    ...
    pub fn initialize(
        ...
        let (mut bank, source) = match initial_state {
            jetstreamer_historical_protocol::InitialState::SnapshotArchive { archive_path } => {
                ...
                let (archive_slot, _) = snapshot::parse_archive_identity(archive_path_ref)?;
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.0.18 epochs 13-15 candidate requires a snapshot at or after slot {}",
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.0.18 epochs 13-15 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.0.18 epochs 13-15 candidate entry slot {} is outside {}..{}",
...
/// Exact equivalent of v1.0.18 ledger::entry::next_hash. Pulling in
...
/// Restore it with the exact v1.0.18 processors linked into this worker, so
...
fn restore_mainnet_runtime_hooks(bank: &mut Bank, genesis: &GenesisConfig) -> Result<(), String> {
    ...
    // The exact upstream Stable callback has no state effects in epochs 13-15.
    // Install the equivalent no-op explicitly and guard the candidate's upper
    // epoch; entry validation rejects a later slot before constructing it.
    bank.set_entered_epoch_callback(Box::new(|bank| {
        assert!(
            bank.epoch() <= MAX_SUPPORTED_EPOCH,
            "Solana v1.0.18 candidate entered unsupported epoch {}",
            bank.epoch()
        );
    }));
    Ok(())
...
mod tests {
    ...
    fn old_form_vote_initialize_transaction(
        ...
        let instructions = vec![
            ...
            // list. v1.0.7 accepted that form; v1.0.8 and v1.0.18 reject it.
            Instruction::new(
                solana_vote_program::id(),
    ...
    #[test]
    fn v1_0_18_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_0_18_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_5615729")
            .expect("set JETSTREAMER_SNAPSHOT_5615729");
        let ledger =
