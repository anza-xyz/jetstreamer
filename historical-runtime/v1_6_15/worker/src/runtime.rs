...
// The epoch-173 terminal checkpoint is the bootstrap for this candidate.
// Admit entries only for epochs 174 through 200; epoch 201 begins outside
// this independently checkpoint-gated v1.6 candidate envelope.
...
// is not treated as evidence that v1.6.15 execution semantics apply out of era.
const MIN_SUPPORTED_SNAPSHOT_SLOT: u64 = 75_167_662;
const MIN_SUPPORTED_ENTRY_SLOT: u64 = MIN_SUPPORTED_SNAPSHOT_SLOT + 1;
const MAX_SUPPORTED_SLOT_EXCLUSIVE: u64 = 86_832_000;
...
impl RuntimeState {
    pub fn initialize(
        ...
        let (bank, source) = match initial_state {
            ...
            jetstreamer_historical_protocol::InitialState::Genesis => {
                return Err(format!(
                    "Solana v1.6.15 candidate requires a snapshot in {}..{}",
        ...
        F: FnMut(EntryProcessed) -> Result<(), E>,
    {
        ...
        let demote_sysvar_write_locks = self.bank.demote_sysvar_write_locks();
        let prepared_results: Vec<Result<PreparedEntry, String>> = self.poh_pool.install(|| {
            requests
                .into_par_iter()
                .map(|request| prepare_entry(request, demote_sysvar_write_locks))
                .collect()
        });
    ...
    fn commit_prevalidated_entry(
        ...
        if transactions.is_empty() {
            ...
            let batch = self.bank.prepare_batch(transactions.iter());
...
fn validate_candidate_snapshot_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_SNAPSHOT_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.6.15 candidate snapshot slot {} is outside {}..{}",
...
fn validate_candidate_entry_slot(slot: u64) -> Result<(), String> {
    if slot < MIN_SUPPORTED_ENTRY_SLOT || slot >= MAX_SUPPORTED_SLOT_EXCLUSIVE {
        return Err(format!(
            "Solana v1.6.15 candidate entry slot {} is outside {}..{}",
...
fn prepare_entry(
    ...
    let signatures_by_writable_key =
        writable_key_attribution(&transactions, demote_sysvar_write_locks)?;
    Ok(PreparedEntry {
        request,
        hash,
        transactions,
...
/// Exact equivalent of v1.6.15 ledger::entry::next_hash. Pulling in
...
/// Exact additional loader table passed by the v1.6.15 ledger restore path on
...
mod tests {
    ...
    #[test]
    fn v1_6_15_rejects_old_form_vote_initialization_without_node_instruction_account() {
        let leader = Pubkey::new_from_array([7; 32]);
        let mut genesis = create_genesis_config_with_leader(1_000_000, &leader, 500_000);
    ...
    #[test]
    #[ignore]
    fn initializes_and_checkpoints_v1_6_15_boundary_snapshot() {
        let archive = std::env::var("JETSTREAMER_SNAPSHOT_75167662")
            .expect("set JETSTREAMER_SNAPSHOT_75167662");
        let ledger =
